...
class _DiscordBotApiClient implements DiscordBotApiClient {
  ...
  Future<List<UserRole>> fetchUserRoles() async {
    ...
    final _result =
      ...
      extra: _extra,
            ...
            .compose(
              _dio.options,
              'https://localhost:5001/api/Get/UserRole',
              queryParameters: queryParameters,
              data: _data,
            )
            .copyWith(baseUrl: baseUrl ?? _dio.options.baseUrl)));
  ...
  @override
  Future<List<UserManage>> fetchUserManage() async {
    const _extra = <String, dynamic>{};
    final queryParameters = <String, dynamic>{};
    final _headers = <String, dynamic>{};
    final _data = <String, dynamic>{};
    final _result = await _dio
        .fetch<List<dynamic>>(_setStreamType<List<UserManage>>(Options(
      method: 'GET',
      headers: _headers,
      extra: _extra,
    )
            .compose(
              _dio.options,
              'https://localhost:5001/api/Get/UserManage',
              queryParameters: queryParameters,
              data: _data,
            )
            .copyWith(baseUrl: baseUrl ?? _dio.options.baseUrl)));
    var value = _result.data!
        .map((dynamic i) => UserManage.fromJson(i as Map<String, dynamic>))
        .toList();
    return value;
  }
